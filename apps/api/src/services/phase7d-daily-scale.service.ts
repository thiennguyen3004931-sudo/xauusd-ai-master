...
type TradeResult = {
  ...
  plus20Reached: boolean;
  firstPartialApplied: boolean;
  secondPartialApplied: boolean;
  firstPartialPnl: number;
  secondPartialPnl: number;
  runnerPnl: number;
  beStopBefore10: boolean;
};
...
type Outcome = TradeResult & {
  ...
  side: Side;
  pattern: Pattern;
  mode: "CURRENT" | "RECOVERY" | "TREND" | "BLOCKED_POSITIVE_LOCK";
  blocked: boolean;
  initialRiskUsd: number;
...
export async function runPhase7DDailyScaleResearch(input: Phase7DDailyScaleRequest) {
  ...
  return {
    ...
    configuration: {
      ...
      filledCandidates: prepared.length,
      accountLogin: health.accountLogin,
      server: health.server,
      volumeStep: spec.volumeStep,
      firstPartialVolume: round(executableOneThird(fixedVolume, spec), 4),
      secondPartialVolume: round(executableOneThird(fixedVolume, spec), 4),
      finalRunnerVolume: round(fixedVolume - 2 * executableOneThird(fixedVolume, spec), 4),
    },
    current,
    recoveryLockCurrent,
    scaleBe6,
    ...
    decision,
    notes: [
      "CURRENT is the existing Phase 7B research management: +6 BE, +10 close one-third, remaining two-thirds runner.",
      "RECOVERY_LOCK_CURRENT adds dynamic full-close Recovery 6-10 while the day is negative and Positive Lock after the day is positive, but keeps current trend management.",
      "RECOVERY_LOCK_SCALE_BE6 keeps Recovery/Lock and uses +6 BE, +10 close one-third, +20 close a second one-third, final one-third runner.",
      "RECOVERY_LOCK_SCALE_BE10 is identical except trend-mode BE is delayed to +10. Recovery mode still protects at +6 before its dynamic 6-10 full-close target.",
      "Positive Lock blocks a new trend-mode trade when its full initial SL risk could reduce an already-positive trading day to the configured floor or below.",
      "M5 OHLC uses STOP_FIRST when stop and favorable trigger coexist inside one M5 bar. Commission, swap and exact tick-level slippage are not reconstructed.",
...
function simulateLane(candidates: PreparedSignal[], lane: LaneConfig, common: Common) {
  ...
  const getDay = (timestamp: number) => {
    ...
    if (!state) {
      state = { day: key, pnl: 0, trades: 0, blocked: 0, wentNegative: false };
      days.set(key, state);
    }
    return state;
  };
  ...
  for (const candidate of candidates) {
    if (candidate.signalTimestamp < busyUntil) {
    ...
    if (
      ...
      dayPnlBeforeEntry - candidate.initialRiskUsd <= common.positiveLockFloorUsd
    ) {
      entryDay.blocked += 1;
      outcomes.push({
        entryTime: candidate.entryTime,
        exitTime: candidate.entryTime,
        exit: candidate.entry,
        side: candidate.side,
        ...
        blocked: true,
        initialRiskUsd: round(candidate.initialRiskUsd, 2),
        dayPnlBeforeEntry: round(dayPnlBeforeEntry, 2),
        recoveryTargetPriceMove: null,
        plus10Reached: false,
        plus20Reached: false,
        firstPartialApplied: false,
        secondPartialApplied: false,
        firstPartialPnl: 0,
        secondPartialPnl: 0,
        runnerPnl: 0,
        beStopBefore10: false,
      });
      continue;
    }

    const result = recoveryMode
      ? simulateRecovery(candidate, clamp(
          (-dayPnlBeforeEntry + common.profitBufferUsd) / (common.cashPerPriceUnitPerLot * candidate.volume),
          common.recoveryMinPrice,
          common.recoveryMaxPrice,
        ), common)
    ...
    outcomes.push({
      ...
      side: candidate.side,
      pattern: candidate.pattern,
      mode: lane.name === "CURRENT" ? "CURRENT" : recoveryMode ? "RECOVERY" : "TREND",
      blocked: false,
      initialRiskUsd: round(candidate.initialRiskUsd, 2),
      dayPnlBeforeEntry: round(dayPnlBeforeEntry, 2),
      recoveryTargetPriceMove: recoveryMode
        ? round(clamp(
            (-dayPnlBeforeEntry + common.profitBufferUsd) / (common.cashPerPriceUnitPerLot * candidate.volume),
            common.recoveryMinPrice,
            common.recoveryMaxPrice,
  ...
  const dayRows = [...days.values()]
    ...
    .map((day) => ({ ...day, pnl: round(day.pnl, 2), recoveredFromNegative: day.wentNegative && day.pnl > 0 }));

  return {
    lane: lane.name,
    metrics: summarize(outcomes, dayRows, skippedPositionBusy),
    days: dayRows.slice(-370).reverse(),
...
function simulateRecovery(candidate: PreparedSignal, targetMove: number, common: Common): TradeResult {
  ...
  for (let index = start; index < common.m5.length; index += 1) {
    ...
    if (stopTouched(candidate.side, bar, activeStop)) {
      return basicResult(candidate, bar.closeTime, activeStop, common, beApplied ? "RECOVERY_BE" : "RECOVERY_STOP");
    }
    ...
    if (!beApplied && favorableMove(candidate.side, candidate.entry, bar) >= 6) {
      activeStop = improveStop(candidate.side, activeStop, candidate.entry);
      beApplied = true;
    }
  }
...
function simulateTrend(candidate: PreparedSignal, config: TrendConfig, common: Common): TradeResult {
  ...
  let plus20Reached = false;
  let firstPartialApplied = false;
  let secondPartialApplied = false;
  let firstPartialPnl = 0;
  let secondPartialPnl = 0;
  let lastReversalM15CloseChecked = candidate.signalTimestamp;
  ...
  for (let index = start; index < common.m5.length; index += 1) {
    ...
    if (stopTouched(candidate.side, bar, activeStop)) {
      ...
      return finish(candidate, bar.closeTime, activeStop, firstPartialPnl + secondPartialPnl + runnerPnl, "STOP",
        plus10Reached, plus20Reached, firstPartialApplied, secondPartialApplied, firstPartialPnl, secondPartialPnl, runnerPnl,
    ...
    if (!beApplied && favorable >= config.beTrigger) {
      activeStop = improveStop(candidate.side, activeStop, candidate.entry);
      beApplied = true;
    }
    ...
    if (!firstPartialApplied && favorable >= FIRST_PARTIAL_TRIGGER) {
      ...
      if (oneThird > 0 && remainingVolume - oneThird >= common.spec.minVolume - 1e-9) {
        firstPartialApplied = true;
        firstPartialPnl = FIRST_PARTIAL_TRIGGER * common.cashPerPriceUnitPerLot * oneThird;
    ...
    if (config.secondPartial && !secondPartialApplied && favorable >= SECOND_PARTIAL_TRIGGER) {
      plus20Reached = true;
      if (oneThird > 0 && remainingVolume - oneThird >= common.spec.minVolume - 1e-9) {
        secondPartialApplied = true;
        secondPartialPnl = SECOND_PARTIAL_TRIGGER * common.cashPerPriceUnitPerLot * oneThird;
        remainingVolume = normalizeVolume(remainingVolume - oneThird, common.spec.volumeStep);
      }
    }
      ...
      if (currentM15Index >= 2) {
        ...
        if (currentM15.closeTime > lastReversalM15CloseChecked && currentM15.closeTime > candidate.signalTimestamp) {
          ...
          if (opposingFvgRejectionAt(candidate.side, common.m15, currentM15Index, REVERSAL_FVG_LOOKBACK)) {
            ...
            return finish(candidate, currentM15.closeTime, exit, firstPartialPnl + secondPartialPnl + runnerPnl,
              "REVERSAL_FVG_REJECTION", plus10Reached, plus20Reached, firstPartialApplied, secondPartialApplied,
              firstPartialPnl, secondPartialPnl, runnerPnl, false);
          }
        }
      }
    }
  ...
  return finish(candidate, candidate.fallbackExitTime, candidate.fallbackExitPrice, firstPartialPnl + secondPartialPnl + runnerPnl,
    "TREND_MA20", plus10Reached, plus20Reached, firstPartialApplied, secondPartialApplied,
    firstPartialPnl, secondPartialPnl, runnerPnl, false);
}

function basicResult(candidate: PreparedSignal, exitTime: number, exit: number, common: Common, reason: string): TradeResult {
  return finish(candidate, exitTime, exit,
    pnlUsd(candidate.side, candidate.entry, exit, candidate.volume, common.cashPerPriceUnitPerLot), reason,
    false, false, false, false, 0, 0,
    pnlUsd(candidate.side, candidate.entry, exit, candidate.volume, common.cashPerPriceUnitPerLot), false);
}
...
function finish(
  ...
  return {
    ...
    plus20Reached,
    firstPartialApplied,
    secondPartialApplied,
    firstPartialPnl: round(firstPartialPnl, 2),
    secondPartialPnl: round(secondPartialPnl, 2),
    runnerPnl: round(runnerPnl, 2),
    beStopBefore10,
  };
}
...
function evaluateDecision(base: ReturnType<typeof summarize>, be6: ReturnType<typeof summarize>, be10: ReturnType<typeof summarize>, sample: number) {
  const candidates = [
    score("RECOVERY_LOCK_SCALE_BE6", base, be6),
    score("RECOVERY_LOCK_SCALE_BE10", base, be10),
  ].sort((a, b) => b.score - a.score);
  const best = candidates[0]!;
  const sufficientSample = sample >= 100 && base.activeDays >= 30;
  const hardPass = best.metrics.netPnl > base.netPnl &&
    (best.metrics.profitFactor ?? 999) >= 1 &&
    best.metrics.maxDrawdownUsd <= base.maxDrawdownUsd * 1.2 + 1e-9 &&
    best.metrics.positiveDayRatePercent >= base.positiveDayRatePercent;
  return {
    sampleTrades: sample,
    sampleDays: base.activeDays,
    sufficientSample,
    verdict: !sufficientSample ? "INSUFFICIENT_SAMPLE" : hardPass ? "SCALE_RESEARCH_PROMISING" : "KEEP_CURRENT_RECOVERY_LOCK",
    preferredResearchLane: sufficientSample && hardPass ? best.lane : "RECOVERY_LOCK_CURRENT",
    executionEligible: false,
    candidates,
    reason: !sufficientSample
      ? "Need at least 100 candidates and 30 active days."
      : hardPass
        ? "A +10/+20 scale lane improves Recovery+Lock economics without worsening drawdown by more than 20% or reducing positive-day rate."
...
function score(lane: string, base: ReturnType<typeof summarize>, metrics: ReturnType<typeof summarize>) {
  ...
  if (metrics.maxDrawdownUsd <= base.maxDrawdownUsd) score += 20;
  else if (metrics.maxDrawdownUsd <= base.maxDrawdownUsd * 1.1) score += 10;
  ...
  return {
    ...
    deltas: {
      ...
      positiveDayRatePercent: round(metrics.positiveDayRatePercent - base.positiveDayRatePercent, 2),
      expectancy: round(metrics.expectancy - base.expectancy, 4),
    },
  };
}
...
function detectPattern(bars: Bar[], index: number): { side: Side; pattern: Pattern; patternExtreme: number } | null {
  ...
  const priorBody = bodySize(prior);
  const firstBody = bodySize(first);
  const combined = firstBody + bodySize(current);
...
function findTrendExit(signal: Signal, m15: Bar[], closeTimes: number[], ma20: Array<number | null>) {
  ...
  for (let index = start + 1; index < m15.length; index += 1) {
    ...
    if (signal.side === "BUY" ? bar.close < average! : bar.close > average!) {
      return { timestamp: bar.closeTime, price: closePriceForSide(signal.side, bar.close, bar.spread) };
    }
  }
  return null;
}
...
function opposingFvgRejectionAt(side: Side, bars: Bar[], currentIndex: number, lookback: number): boolean {
  ...
  for (let index = currentIndex - 1; index >= start; index -= 1) {
    ...
    if (side === "BUY" && third.high < first.low && current.high >= third.high && current.low <= first.low && current.close < first.low) return true;
    if (side === "SELL" && third.low > first.high && current.high >= third.low && current.low <= third.low && current.close > first.high) return true;
  }
  return false;
}
...
function latestConfirmedStructure(side: Side, after: number, atOrBefore: number,
  lows: Array<{ confirmedAt: number; level: number }>, highs: Array<{ confirmedAt: number; level: number }>): number | null {
...
function executableOneThird(volume: number, spec: Spec): number {
  const value = floorToStep(volume / 3, spec.volumeStep);
  if (value < spec.minVolume - 1e-9) return 0;
  return value;
}

function targetTouched(side: Side, entry: number, move: number, bar: Bar): boolean {
  return side === "BUY" ? bar.high >= entry + move - 1e-9 : bar.low + bar.spread <= entry - move + 1e-9;
}
...
function closePriceForSide(side: Side, bid: number, spread: number): number { return side === "BUY" ? bid : bid + spread; }
...
function pnlUsd(side: Side, entry: number, exit: number, volume: number, cash: number): number {
  return (side === "BUY" ? exit - entry : entry - exit) * cash * volume;
}
function validateFixedVolume(volume: number, spec: Spec) {
  ...
  if (oneThird <= 0 || volume - 2 * oneThird < spec.minVolume - 1e-9) {
    throw new Error(`fixedVolume ${volume} cannot support +10/+20 one-third scale with broker step ${spec.volumeStep}.`);
  }
}
...
function dayKey(timestamp: number, offset: number): string { return new Date(timestamp + offset * 3_600_000).toISOString().slice(0, 10); }
