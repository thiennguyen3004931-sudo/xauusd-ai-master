...
import { getPhase7CPerformanceEffectivenessSnapshot } from "../services/phase7c-performance-effectiveness.service";
...
router.get("/", async (req: Request, res: Response) => {
  if (!isLoopbackRequest(req)) {
    res.status(403).json({ error: "Performance effectiveness is restricted to localhost." });
    return;
  }
  try {
    ...
    res.json(
      await getPhase7CPerformanceEffectivenessSnapshot({
    ...
    res.status(400).json({
      error: error instanceof Error
        ? error.message
        : "Could not build Phase7C performance effectiveness snapshot.",
    });
  }
});
