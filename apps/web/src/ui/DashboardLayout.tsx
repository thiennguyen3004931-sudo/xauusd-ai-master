...
const researchLinks = [
  ...
  ["/phase7e-realignment", "ST M5 Re-alignment", TuneRounded],
...
const performanceLinks = [["/performance", "Forward Performance", InsightsRounded]] as const;
const riskLinks = [["/phase7c-risk", "Risk & Auto Lot", ShieldRounded]] as const;
const systemLinks = [["/system", "System Health", DnsRounded]] as const;
const legacyLinks = [
...
function NavGroup({ title, links, onNavigate }: { title: string; links: readonly LinkRow[]; onNavigate?: () => void }) {
  return (
    <Box sx={{ mt: 1.2 }}>
      <Typography variant="caption" color="text.disabled" sx={{ px: 1.5, letterSpacing: ".12em", fontWeight: 800 }}>{title}</Typography>
      <List dense sx={{ pt: .4 }}>
        {links.map(([href, label, Icon]) => (
          <ListItemButton key={href} component={NavLink} to={href} end={href === "/"} onClick={onNavigate} sx={{ my: .35, borderRadius: 2, color: "text.secondary", "&.active": { color: "text.primary", backgroundColor: "rgba(233,185,73,.09)", border: "1px solid rgba(233,185,73,.14)" } }}>
  ...
  return (
    <Box sx={{ height: "100%", display: "flex", flexDirection: "column", p: 2, overflowY: "auto" }}>
      <Stack direction="row" spacing={1.5} alignItems="center" sx={{ px: 1, py: 1.5 }}>
        ...
        <Box><Typography variant="caption" color="primary" sx={{ letterSpacing: ".18em" }}>XAUUSD</Typography><Typography variant="subtitle2" fontWeight={800}>AI MASTER</Typography></Box>
      </Stack>
...
export function DashboardLayout() {
  ...
  const isPhase7ERealignment = location.pathname.startsWith("/phase7e-realignment");
  ...
  const isOperational = isControlCenter || isPhase7B || isPatternCheck || isPhase7BOps || isPhase7CBacktest || isPhase7DDailyPnl || isPhase7DManagement || isPhase7DDailyScale || isPhase7ESupertrend || isPhase7ERealignment || isPhase7CAutoLot || isPhase7CRisk || isPerformance || isSystem;
  ...
  let headerSubtitle = dashboard.isError ? "Legacy research API unavailable · Phase 7B DEMO is independent" : "Legacy research snapshot · polling 5s";
  ...
  else if (isPhase7DDailyPnl) { headerTitle = "Phase 7D Daily P/L Optimizer"; headerSubtitle = "Baseline vs Recovery vs Positive Lock combinations · research only"; headerMode = "RESEARCH"; }
  else if (isPhase7DManagement) { headerTitle = "Phase 7D BE + Partial Optimizer"; headerSubtitle = "+6 BE vs +10 BE · one-third vs theoretical half · exact per-variant contention"; headerMode = "RESEARCH"; }
  else if (isPhase7DDailyScale) { headerTitle = "Phase 7D Daily 10/20 Scale Optimizer"; headerSubtitle = "Negative-day Recovery 6–10 · Positive Lock · +10/+20 thirds · final runner"; headerMode = "RESEARCH"; }
  else if (isPhase7CAutoLot) { headerTitle = "Auto Lot SHADOW vs Fixed"; headerSubtitle = "Risk-based sizing overlay · exact one-third management compatibility · no execution mutation"; headerMode = "SHADOW"; }
  else if (isPhase7CRisk) { headerTitle = "Risk & Auto Lot SHADOW"; headerSubtitle = "Broker-native XAUUSD sizing · no execution mutation"; headerMode = "SHADOW"; }
  else if (isPerformance) { headerTitle = "MT5 DEMO Performance"; headerSubtitle = "System-owned forward results · read-only analytics"; headerMode = "DEMO ONLY"; }
  else if (isSystem) { headerTitle = "DEMO System Health"; headerSubtitle = "MT5 / Bridge / Phase 7B operational telemetry"; headerMode = "DEMO ONLY"; }

  return (
    <Box sx={{ display: "flex", minHeight: "100vh" }}>
      <Drawer variant="permanent" sx={{ width: drawerWidth, display: { xs: "none", lg: "block" }, "& .MuiDrawer-paper": { width: drawerWidth, bgcolor: "#08111f", borderRightColor: "rgba(148,163,184,.12)" } }}><Navigation /></Drawer>
      <Drawer open={mobileOpen} onClose={() => setMobileOpen(false)} sx={{ display: { xs: "block", lg: "none" }, "& .MuiDrawer-paper": { width: drawerWidth, bgcolor: "#08111f" } }}><Navigation onNavigate={() => setMobileOpen(false)} /></Drawer>
      <Box sx={{ flex: 1, minWidth: 0 }}>
        <AppBar position="sticky" elevation={0} sx={{ bgcolor: "rgba(6,11,20,.88)", backdropFilter: "blur(14px)", borderBottom: "1px solid rgba(148,163,184,.10)" }}>
          <Toolbar>
            ...
            <Box sx={{ flex: 1 }}><Typography variant="subtitle2" fontWeight={800}>{headerTitle}</Typography><Typography variant="caption" color="text.secondary">{headerSubtitle}</Typography></Box>
