...
function replaceRegexAll(source, regex, replacer, label, expected) {
  const matches = [...source.matchAll(regex)];
  if (matches.length !== expected) {
    throw new Error(`${label}: expected ${expected} regex matches, found ${matches.length}`);
  }
...
update("scripts/run-phase7b-telegram-notifier.mjs", (input) => {
  ...
  source = replaceRegexAll(
    source,
    /function holdKey\(event\) \{[\s\S]*?\n\}\n\nfunction shouldSuppressHoldAddon/g,
...
update("scripts/run-phase7b-demo-controller.ts", (input) => {
  ...
  source = replaceRegexAll(
    source,
    /`\$\{managed\.ticket\}\|\$\{hold\.reasonCode\}`/g,
  ...
  source = replaceRegexAll(
    source,
    /hold\s*&&\s*holdKey\s*!==\s*lastHoldObservationKey\s*\)\s*\{\s*lastHoldObservationKey\s*=\s*holdKey;/g,
...
console.log("PHASE7C_HOLD_M15_PATCH_V4=APPLIED");
