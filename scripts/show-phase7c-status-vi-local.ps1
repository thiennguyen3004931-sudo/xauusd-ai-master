...
Write-Host ("Tài khoản           : {0}" -f $decision.account.accountMode)
...
Write-Host ("Điều phối            : {0}" -f (Get-PidAliveVi $RuntimeDir "supervisor"))
Write-Host ("Bot Trend           : {0}" -f (Get-PidAliveVi $RuntimeDir "trend"))
Write-Host ("Bot Sideway         : {0}" -f (Get-PidAliveVi $RuntimeDir "sideway"))
Write-Host ("Điều khiển Telegram : {0}" -f (Get-PidAliveVi $RuntimeDir "telegram-mode"))
Write-Host ("Thông báo thị trường: {0}" -f (Get-PidAliveVi $RuntimeDir "regime-notifier"))
