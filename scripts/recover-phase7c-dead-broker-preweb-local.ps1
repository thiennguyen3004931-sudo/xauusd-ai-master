...
Write-Host '============================================================'
Write-Host '=== PHASE7C DEAD BROKER PRE-WEB RECOVERY ==='
Write-Host '============================================================'
...
if ([string]$deployment.sourceCommit -ne $ExpectedCommit -or `
    [string]$deployment.sourceTree -ne $sourceTree -or `
    [string]$deployment.branch -ne 'main' -or `
    -not [bool]$deployment.worktreeClean) {
  throw 'Dead-broker recovery requires the current accepted deployment to match exact main source.'
}
