...
export function evaluateTimestampFreshness(timestamp, {
  ...
  let offset;
  if (clockOffsetMs === null || clockOffsetMs === undefined) {
    // MT5 brokers commonly expose server-local epochs (for example UTC+2/+3).
    // Infer only a plausible whole-hour skew. The residual tolerance expands
    // to the allowed freshness window so a closed M15 candle several minutes
    // old can still share the same broker offset as the live quote.
    offset = inferBrokerClockOffset(value, {
      systemTimestamp: current,
      residualToleranceMs: Math.max(5 * 60_000, maximumAge + Math.max(0, futureTolerance)),
    });
    if (offset === null) offset = 0;
  } else {
    offset = Number(clockOffsetMs);
...
export function validateAutoLotSnapshot(payload, {
  ...
  // Auto Lot is generated by the local control API, not MT5, so force zero
  // broker offset rather than auto-inferring one from a stale snapshot.
