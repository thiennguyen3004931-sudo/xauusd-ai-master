...
Write-Host "PHASE7B_V24_ACCOUNT_LOGIN=$($health.accountLogin)"
Write-Host "PHASE7B_V24_ACCOUNT_MODE=$($health.accountMode)"
Write-Host "PHASE7B_V24_SERVER=$($health.server)"
Write-Host "PHASE7B_V24_ACCOUNT_BALANCE=$($health.accountBalance)"
Write-Host "PHASE7B_V24_DAY_START_LOCAL=$(To-LocalTimestamp $boundary.currentStartTime)"
Write-Host "PHASE7B_V24_NOW_LOCAL=$(To-LocalTimestamp $toMs)"
Write-Host "PHASE7B_V24_DAY_XAUUSD_DEALS=$($dayDeals.Count)"
Write-Host "PHASE7B_V24_EXTENDED_ALL_DEALS=$($extendedDeals.Count)"
Write-Host "PHASE7B_V24_LOOKBACK_HOURS=$LookbackHours"
Write-Host "PHASE7B_V24_TARGET_ID=$TargetId"
...
if ($targetDeals.Count -eq 0) {
  Write-Host "PHASE7B_V24_TARGET_MT5_FOUND=False"
} else {
  Write-Host "PHASE7B_V24_TARGET_MT5_FOUND=True"
  $targetRows = foreach ($deal in $targetDeals) {
    [pscustomobject]@{
      LocalTime = To-LocalTimestamp $deal.timestamp
      Ticket = [string]$deal.ticket
      OrderId = [string]$deal.orderId
  ...
  $targetRows | Format-List
}

Write-Host "`n=== EXTENDED XAUUSD BOT-MAGIC DEALS ==="
$extendedBotXau = @(
  $extendedDeals | Where-Object {
    [string]$_.symbol -eq "XAUUSD" -and [string]$_.magic -eq "270713"
  }
)
$extendedBotRows = foreach ($deal in $extendedBotXau) {
  [pscustomobject]@{
    LocalTime = To-LocalTimestamp $deal.timestamp
    Ticket = [string]$deal.ticket
    OrderId = [string]$deal.orderId
...
$extendedBotRows | Sort-Object LocalTime | Format-Table -AutoSize
...
if ($journalMatches.Count -eq 0) {
  Write-Host "PHASE7B_V24_TARGET_JOURNAL_FOUND=False"
} else {
  Write-Host "PHASE7B_V24_TARGET_JOURNAL_FOUND=True"
...
Write-Host "PHASE7B_V24_V20_POSITION_COUNT=$($closedBotPositionIds.Count)"
Write-Host "PHASE7B_V24_V20_REALIZED_PNL=$([math]::Round($v20Pnl,2))"
...
try {
  ...
  Write-Host "PHASE7B_V24_API_READ=FAIL $($_.Exception.Message)"
}

Write-Host "PHASE7B_V24_READ_ONLY=True"
Write-Host "PHASE7B_V24_ORDER_PERMISSION=NONE"
Write-Host "PHASE7B_V24_REAL_ACCOUNT_ALLOWED=False"
Write-Host "PHASE7B_V24=PASS"
