...
try {
  ...
  if ($head -ne $ExpectedCommit) {
    throw "Web UI deploy requires exact HEAD $ExpectedCommit. Current=$head. Sync main to the intended merged SHA first."
  }
  ...
  Write-Host "PHASE7C_WEB_UI_DEPLOY_HEAD=$head"
