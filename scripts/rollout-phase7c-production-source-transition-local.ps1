...
foreach ($value in @($ExpectedCurrentCommit, $TargetCommit)) {
    if ($value -notmatch '^[0-9a-f]{40}$') {
        throw 'ExpectedCurrentCommit and TargetCommit must be exact 40-character Git SHAs.'
    }
}
if ($ExpectedCurrentCommit -eq $TargetCommit) {
    throw 'Production source transition requires a distinct target commit.'
}
if ($ExpectedHelperBlobSha1 -notmatch '^[0-9a-f]{40}$') {
    throw 'ExpectedHelperBlobSha1 must be an exact 40-character Git blob SHA-1.'
}
...
Write-Host "PHASE7C_PRODUCTION_SOURCE_TRANSITION_HELPER_PROVENANCE=PASS|BLOB=$actualHelperBlob"

foreach ($required in @($ConfigPath, $AccountLibrary, $OwnershipLibrary)) {
    if (-not (Test-Path -LiteralPath $required -PathType Leaf)) {
        throw "Required current-generation rollout dependency is missing: $required"
    }
}
...
function Get-ArmCapability {
    return Invoke-ApiGet '/api/v1/phase7c-live-arm-control/capability'
}

function Assert-Arm([ValidateSet('ARMED','DISARMED')] [string]$Expected, [string]$Stage) {
    $arm = Get-ArmCapability
    if ([string]$arm.accountMode -ne 'LIVE') { throw "$Stage requires LIVE ARM capability." }
    if ([string]$arm.liveArmStatus -ne $Expected) {
        throw "$Stage ARM mismatch. expected=$Expected actual=$($arm.liveArmStatus)"
    }
    if ($Expected -eq 'ARMED' -and -not [bool]$arm.liveExecutionArmed) {
        throw "$Stage expected liveExecutionArmed=true."
    }
    if ($Expected -eq 'DISARMED' -and [bool]$arm.liveExecutionArmed) {
        throw "$Stage expected liveExecutionArmed=false."
    }
}
...
function Assert-RuntimeSourceExact([string]$ExpectedCommit, [string]$ExpectedTree, [string]$Stage) {
    ...
    if ([string]$snapshot.overall -ne 'EXACT_MATCH') {
        throw "$Stage runtime-source overall is not EXACT_MATCH. actual=$($snapshot.overall)"
    }
    if ($null -eq $snapshot.deployment) { throw "$Stage runtime-source deployment is missing." }
    if ([string]$snapshot.deployment.sourceCommit -ne $ExpectedCommit) {
        throw "$Stage deployment commit mismatch. expected=$ExpectedCommit actual=$($snapshot.deployment.sourceCommit)"
    }
    if ([string]$snapshot.deployment.sourceTree -ne $ExpectedTree) {
        throw "$Stage deployment tree mismatch. expected=$ExpectedTree actual=$($snapshot.deployment.sourceTree)"
    }
    ...
    foreach ($component in $components) {
        if ([string]$component.verdict -ne 'EXACT_MATCH' -or [string]$component.sourceCommit -ne $ExpectedCommit -or [string]$component.sourceTree -ne $ExpectedTree) {
            throw "$Stage runtime-source component mismatch. component=$($component.component) verdict=$($component.verdict) commit=$($component.sourceCommit)"
        }
    }
...
# Source-control preflight is completed before any LIVE runtime mutation.
...
try {
    ...
    if ($remoteMain -ne $TargetCommit) { throw "origin/main is not the requested target. expected=$TargetCommit actual=$remoteMain" }

    & $gitExe fetch --no-tags origin refs/heads/main:refs/remotes/origin/main
    if ($LASTEXITCODE -ne 0) { throw 'Could not pre-stage canonical origin/main target.' }
    $originMain = ([string](& $gitExe rev-parse refs/remotes/origin/main)).Trim().ToLowerInvariant()
    if ($LASTEXITCODE -ne 0 -or $originMain -ne $TargetCommit) { throw "Pre-staged origin/main mismatch. expected=$TargetCommit actual=$originMain" }
...
Write-Host "PHASE7C_PRODUCTION_SOURCE_TRANSITION_GIT_PREFLIGHT=PASS|CURRENT=$ExpectedCurrentCommit|TARGET=$TargetCommit"
...
try {
    ...
    if ($mutationStarted) {
        ...
        try {
            $arm = Get-ArmCapability
            if ([string]$arm.liveArmStatus -ne 'DISARMED' -or [bool]$arm.liveExecutionArmed) {
                Invoke-LiveArmAction -Action 'DISARM_LIVE'
            }
