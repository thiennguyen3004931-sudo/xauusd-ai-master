...
// Bootstrap V13 when needed: safe recovery target + compact wrapper + open sync.
const needV13 =
...
// A. Recovery clamp must be safe before Bot restart.
...
console.log("PHASE7B_V15_DAILY_RECOVERY_CLAMP=PASS");

// B. Telegram wrapper must use compact notifier.
...
console.log("PHASE7B_V15_TELEGRAM_WRAPPER=COMPACT");

// C. Never suppress broker SL close merely because Telegram started late.
...
// D. Seed notifier state when syncing an already-open position.
...
// E. Close formatter can recover side from MANAGED_POSITION_CLOSED.lastKnownState.
compact = compact.replace(
...
// F. On notifier startup with no open position, replay only the latest missed
// close if its timestamp is newer than state.lastEventAt.
...
console.log("PHASE7B_V15_TELEGRAM_CLOSE_SUPPRESSION=False");
console.log("PHASE7B_V15_TELEGRAM_CLOSE_FALLBACK=LAST_KNOWN_STATE_PLUS_MT5_HISTORY");
console.log("PHASE7B_V15_TELEGRAM_MISSED_CLOSE_REPLAY=PASS");
console.log("PHASE7B_V15_TELEGRAM_ORDER_PERMISSION=READ_ONLY");
console.log("PHASE7B_V15_BOT_RESTART_REQUIRED=True");
console.log("PHASE7B_V15_REAL_ACCOUNT_ALLOWED=False");
console.log("PHASE7B_V15=PASS");
