...
# historical_candles imports Mt5Gateway only for typing. Stub that dependency so
# this read-only regression remains stdlib-only and does not require the
# Windows-only MetaTrader5 package in Linux CI.
...
class _Gateway:
    ...
    def _read_with_reconnect_locked(self, method_name: str, *args: object) -> object:
...
class HistoricalCandleBrokerTimeTest(unittest.TestCase):
    def test_utc_range_is_denormalized_to_broker_clock_before_copy_rates_range(self) -> None:
        ...
        try:
            ...
            symbol, timeframe, start, end = gateway.captured["args"]  # type: ignore[misc]
